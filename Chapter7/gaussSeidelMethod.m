function [x,it] = gaussSeidelMethod(A,b,x0,itMax,eps)
%   Using the Gauss-Seidel method to iteratively solve the linear system Ax = b
%   
%   Input:
...
%
    ...
    while it < itMax && err > eps
        for j = 1 : n % caculate all compoenets of x^(k+1)
            ...
            sum = 0;
            for k = 1:n
                if k ~= j % skip x1 in first equation, x2 in second, ...
                    sum = sum + x(k).*A(j,k)
                    x(j) =  (1 ./ A(j,j)) .* (b(j) - sum)
                end
            end
